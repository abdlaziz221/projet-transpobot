...
# ── Connexion MySQL ────────────────────────────────────────────
def get_db():
    return mysql.connector.connect(**DB_CONFIG, charset='utf8mb4', collation='utf8mb4_unicode_ci')

def validate_sql(sql: str):
    sql_upper = sql.strip().upper()
    forbidden = ["DELETE", "DROP", "UPDATE", "INSERT", "ALTER", "GRANT", "TRUNCATE"]
    if not sql_upper.startswith("SELECT") or any(word in sql_upper for word in forbidden):
        raise HTTPException(status_code=403, detail="Requête non autorisée : Seuls les SELECT sont autorisés.")
    return True

def execute_query(sql: str):
    validate_sql(sql) # Appel de la sécurité
    conn = get_db()
...
def get_incidents():
    """Retourne la liste des incidents avec les détails des trajets."""
    return execute_query("SELECT i.*, t.date_heure_depart FROM incidents i LEFT JOIN trajets t ON i.trajet_id = t.id")

@app.get("/api/lignes", tags=["Data"])
def get_lignes():
    """Retourne la liste des lignes de transport."""
    return execute_query("SELECT * FROM lignes")

@app.get("/api/tarifs", tags=["Data"])
def get_tarifs():
    """Retourne la grille tarifaire."""
    return execute_query("SELECT t.*, l.nom as ligne FROM tarifs t JOIN lignes l ON t.ligne_id = l.id")

@app.get("/api/export/csv", tags=["Export"])
async def export_csv(sql: str):
...
def save_history(question: str, sql: str):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO conversation_logs (question, sql_genere) VALUES (%s, %s)", (question, sql))
    conn.commit()
    cursor.close()
    conn.close()
@app.get("/api/historique", tags=["Historique"])
def get_historique():
    """Récupère l'historique des requêtes."""
    return execute_query("SELECT * FROM conversation_logs ORDER BY timestamp DESC LIMIT 50")
# Mise à jour de la route /api/chat pour inclure la sauvegarde
